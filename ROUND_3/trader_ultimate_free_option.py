...
EXPIRY_SHORT_START = 998_000
EXPIRY_MIN_EXTRINSIC_BID = 0.75
VOUCHER_STRIKES = {
    "VEV_4000": 4000,
    "VEV_4500": 4500,
    "VEV_5000": 5000,
...
class Trader:
    ...
    # ---- End-of-round exercise arb: short time value before VEV settlement ----
    def trade_expiry_short(self, state, result, S: float):
        if state.timestamp < EXPIRY_SHORT_START:
            return
        for product, strike in VOUCHER_STRIKES.items():
            ...
            if q.bid is None:
                continue
            intrinsic = max(S - strike, 0.0)
            if q.bid <= intrinsic + EXPIRY_MIN_EXTRINSIC_BID:
                continue
            orders = result.setdefault(product, [])
            orders.clear()
            self.sell(state, product, orders, q.bid, POS_LIMITS[product])
    ...
    def run(self, state: TradingState):
        ...
        if store.get("option_risk_off"):
            ...
            self.trade_expiry_short(state, result, S)
            store["prev_S"] = S
