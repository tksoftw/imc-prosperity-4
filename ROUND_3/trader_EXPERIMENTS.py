...
class Trader:
    ...
    def trade_hydro(
        ...
        # Round-3 historical mid centers around 9991. Use it as a light
        # anchor, but let the raw-mid EMAs adapt to the current regime.
        fast = update_ema(store["emas"], f"{HYDRO}_fast", mid, 30)
        slow = update_ema(store["emas"], f"{HYDRO}_slow", mid, 120)
        fair = 0.20 * mid + 0.25 * fast + 0.40 * slow + 0.15 * 9991.0
        fair -= 0.05 * position
