...
class Trader:
    ...
    def option_risk_gauge(self, store: Dict, S: float, smile_lv: float) -> float:
        """Online regime gauge: higher when spot and IV/extrinsic are fading.

        This is deliberately not tied to any clock tick or real-log level.
        It reacts only to the market path seen so far, which makes it a
        general seatbelt against long-call bags in low-vol down moves.
        """
        spot_fast = update_ema(store["emas"], "risk_spot_fast", S, 25)
        spot_slow = update_ema(store["emas"], "risk_spot_slow", S, 180)
        iv_fast = update_ema(store["emas"], "risk_iv_fast", smile_lv, 25)
        iv_slow = update_ema(store["emas"], "risk_iv_slow", smile_lv, 180)

        spot_scale = max(1.0, abs(S) * 0.00035)
        iv_scale = max(0.0004, abs(smile_lv) * 0.025)
        spot_bad = max(0.0, min(1.0, (spot_slow - spot_fast) / spot_scale))
        iv_bad = max(0.0, min(1.0, (iv_slow - iv_fast) / iv_scale))
        risk = max(0.0, min(1.0, 0.65 * spot_bad + 0.35 * iv_bad))
        store["risk"] = risk
        return risk
    ...
    def trade_itm_flash(
        ...
        fair = max(S - strike, 0.0) - 1.25 * risk
        spread = q.spread
        ...
        if position > 0:
            if risk > 0.55 and q.bid is not None:
                self.sell(state, product, orders, q.bid,
                          min(position, q.bid_vol, 8 if strike == 4000 else 6))
    ...
    def trade_5000(self, state, store, result, S, spot_edge: float, risk: float):
    ...
    def trade_smile_atm(
        ...
        fair -= risk * delta * (1.2 if strike == 5100 else 0.9)
        ...
        if q.ask <= fair - take_edge and risk < 0.85:
        ...
        if risk > 0.70 and position > 0 and q.bid is not None:
            self.sell(state, product, orders, q.bid, min(position, q.bid_vol, size))

        if spread >= 2:
            target_bid = int(min(q.bid + 1, math.floor(fair - quote_edge)))
            target_ask = int(max(q.ask - 1, math.ceil(fair + quote_edge)))
            if target_bid < target_ask:
                if spot_edge >= (-1.25 if strike == 5100 else -2.5) and risk < 0.75:
    ...
    def trade_5300(self, state, result, S, vev_dir, risk: float):
        ...
        fair = max(S - strike, 0.0) + 47.0 - 2.5 * risk
        fair -= 0.015 * position
        if vev_dir != 0:
            fair += 2.0 * vev_dir

        if q.ask < fair - 2.0 and risk < 0.75:
            self.buy(state, product, orders, q.ask, min(-q.ask_vol, 20))
        if q.bid > fair + 2.0:
            self.sell(state, product, orders, q.bid, min(q.bid_vol, 20))
        if risk > 0.55 and position > 0 and q.bid is not None:
            self.sell(state, product, orders, q.bid, min(position, q.bid_vol, 10))
    ...
    def trade_5400(self, state, store, result, S, risk: float):
        ...
        fair = max(S - strike, 0.0) + 16.0 - 1.8 * risk
    ...
    def trade_5500(self, state, store, result, S, basket_active: bool, risk: float):
        ...
        fair = intrinsic + ext_blend - 1.2 * risk
        fair += self.option_micro_reversion(store, product, q, S, 0.08, 0.50)

        if basket_active and q.ask <= fair - 0.5 and risk < 0.75:
            self.buy(state, product, orders, q.ask, min(-q.ask_vol, 8))
        elif q.ask <= fair - 2.5 and risk < 0.75:
            self.buy(state, product, orders, q.ask, min(-q.ask_vol, 5))
        if q.bid >= fair + 2.5:
            self.sell(state, product, orders, q.bid, min(q.bid_vol, 5))
        if risk > 0.50 and position > 0 and q.bid is not None:
            self.sell(state, product, orders, q.bid, min(position, q.bid_vol, 6))

        if q.spread is not None and q.spread <= 2 and q.bid <= fair - 0.5 and risk < 0.65:
    ...
    def run(self, state: TradingState):
        ...
        risk = self.option_risk_gauge(store, S, smile_lv)
        ...
        self.trade_itm_flash(state, result, "VEV_4000", 4000, S, surface_shift, wing_signal, risk)
        self.trade_itm_flash(state, result, "VEV_4500", 4500, S, surface_shift, wing_signal, risk)

        self.trade_5000(state, store, result, S, spot_edge, risk)
        self.trade_smile_atm(state, store, result, smile_lv, "VEV_5100", 5100, S, v_active, spot_edge, risk)
        self.trade_smile_atm(state, store, result, smile_lv, "VEV_5200", 5200, S, v_active, spot_edge, risk)

        self.trade_5300(state, result, S, vev_dir, risk)
        self.trade_5400(state, store, result, S, risk)
        self.trade_5500(state, store, result, S, basket_active, risk)
