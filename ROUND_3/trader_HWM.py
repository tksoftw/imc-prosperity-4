"""ROUND_3: trader_HWM — high-watermark risk seatbelt.
...
calibration fuel. Generic PnL high-watermark stop flattens drawdowns.
"""
...
class Trader:
    ...
    def mark_pnl(self, state: TradingState, store: Dict) -> float:
        ...
        for product, trades in state.own_trades.items():
            for tr in trades:
                key = f"{product}:{tr.timestamp}:{tr.price}:{tr.quantity}:{tr.buyer}:{tr.seller}"
                if key in seen:
                    continue
                seen.add(key)
                ...
                if tr.seller == "SUBMISSION":
                    cash[product] += float(tr.price) * qty
        # Bound serialized trade ids; own_trades are recent, this is just safety.
        ...
        for product, c in cash.items():
            ...
            if mid is None:
                continue
            total += float(c) + self.position(state, product) * mid
        high = max(float(store.get("pnl_high", 0.0)), total)
        store["pnl_high"] = high
        # Generic seatbelt: once meaningful money is made, protect large giveback.
        drawdown = high - total
        store["risk_off"] = bool(high > 50000.0 and drawdown > max(12000.0, 0.18 * high))
        return total

    def flatten_all(self, state: TradingState, result: Dict[str, List[Order]]) -> None:
    ...
    def run(self, state: TradingState):
        ...
        if store.get("risk_off"):
            self.flatten_all(state, result)
