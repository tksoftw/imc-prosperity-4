...
class Trader:
    ...
    def flatten_options(self, state: TradingState, result: Dict[str, List[Order]]) -> None:
        for product in (
            ...
            if pos > 0 and q.bid is not None:
                self.sell(state, product, orders, q.bid, min(pos, max(1, q.bid_vol)))
            elif pos < 0 and q.ask is not None:
                self.buy(state, product, orders, q.ask, min(-pos, max(1, -q.ask_vol)))
    ...
    # ---- Bull-call spread free-option check ----
    SPREADS = (
        (4000, 4500, 2.0, 4),
        (4500, 5000, 2.0, 4),
        (5000, 5100, 3.0, 6),
    ...
    def trade_vertical_spread_arb(self, state, result, S: float):
        for k_lo, k_hi, min_edge, max_size in self.SPREADS:
            if S < k_hi:
                continue
            lo = f"VEV_{k_lo}"
            hi = f"VEV_{k_hi}"
            q_lo = quote_from(state.order_depths.get(lo))
            q_hi = quote_from(state.order_depths.get(hi))
            if q_lo.ask is None or q_hi.bid is None:
                continue
            ...
            if avail <= 0:
                continue
            orders_lo = result.setdefault(lo, [])
            orders_hi = result.setdefault(hi, [])
            self.buy(state, lo, orders_lo, q_lo.ask, avail)
            self.sell(state, hi, orders_hi, q_hi.bid, avail)
    ...
    def run(self, state: TradingState):
        ...
        self.trade_vertical_spread_arb(state, result, S)
