"""ROUND_3: trader_VOL — online vol-adaptive wing fair.
...
calibration fuel. Wing anchors are blended with online extrinsic EMAs.
"""
...
class Trader:
    ...
    def trade_5300(self, state, store, result, S, vev_dir):
        ...
        ext_blend = max(37.0, min(53.0, 0.55 * 47.0 + 0.45 * ext_ema))
    ...
    def trade_5400(self, state, store, result, S):
        ...
        observed_ext = q.mid - intrinsic
        ext_ema = update_ema(store["emas"], f"{product}_ext", observed_ext, 120)
        ext_blend = max(10.0, min(20.0, 0.55 * 16.0 + 0.45 * ext_ema))
        fair = intrinsic + ext_blend
