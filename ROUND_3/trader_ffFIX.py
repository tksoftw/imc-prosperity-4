...
class Trader:
    ...
    @staticmethod
    def late_risk_off(state: TradingState) -> bool:
        # Earlier dump threshold = capture more of the day's mid drop.
        return state.timestamp >= 960000
    ...
    def trade_itm_flash(
        ...
        if self.late_risk_off(state):
            if position > 0 and q.bid is not None:
                self.sell(state, product, orders, q.bid, min(position, q.bid_vol, 30))
            return
    ...
    # ---- VERTICAL SPREAD ARB ----
    # Discovery: on day-3 the bull-call spreads are perpetually under-priced
    # vs their max payoff. e.g. VEV_5100 ask - VEV_5200 bid ~ 82 while max
    # payoff (S above 5200) is 100. Free $18 per spread. We BUY the lo strike
    # ASK + SELL the hi strike BID whenever (k_hi - k_lo) - (ask_lo - bid_hi)
    # exceeds a threshold and S sits above k_hi (so payoff caps at k_hi-k_lo).
    SPREADS = (
        # (lo_strike, hi_strike, min_arb_profit, max_size_per_tick)
        (5000, 5100, 3.0, 6),
        (5100, 5200, 5.0, 6),
        (5200, 5300, 8.0, 6),
        (4500, 5000, 1.5, 4),
    ...
    def trade_vertical_spread_arb(self, state, result, S):
        """Long lo + Short hi when the spread is mispriced vs max payoff."""
        if S is None:
            return
        for k_lo, k_hi, min_arb, max_size in self.SPREADS:
            sym_lo = f"VEV_{k_lo}"
            sym_hi = f"VEV_{k_hi}"
            q_lo = quote_from(state.order_depths.get(sym_lo))
            q_hi = quote_from(state.order_depths.get(sym_hi))
            if q_lo.ask is None or q_lo.ask_vol is None or q_hi.bid is None or q_hi.bid_vol is None:
                continue
            # Only when S is above the upper strike — payoff caps at k_hi-k_lo.
            if S < k_hi:
                continue
            cost = q_lo.ask - q_hi.bid
            cap = k_hi - k_lo
            arb_profit = cap - cost
            if arb_profit < min_arb:
                continue
            # Buy lo + Sell hi up to the smaller of the two top-of-book sizes.
            avail = min(-q_lo.ask_vol, q_hi.bid_vol, max_size)
            if avail <= 0:
                continue
            orders_lo = result.setdefault(sym_lo, [])
            orders_hi = result.setdefault(sym_hi, [])
            self.buy(state, sym_lo, orders_lo, q_lo.ask, avail)
            self.sell(state, sym_hi, orders_hi, q_hi.bid, avail)

    # ---- VEV_6000 / VEV_6500: aggressive free-option harvest ----
    # Always max-size 0-bid (300 lots) for queue dominance. Also offer at 1
    # constantly to flip into any speculator that lifts.
    def trade_zero_tail(self, state, result, product, basket_active: bool):
        ...
        # Always max bid at 0 — every dump is free EV.
        self.buy(state, product, orders, 0, POS_LIMITS[product])

        # Always offer inventory at 1 (free upside).
        if position > 0:
            self.sell(state, product, orders, 1, position)

    # ---- VEV_5500 also has wing dumps at price 0/1 (62 events on day 3) ----
    # Stack low bids in queue to catch them. Sell back at mid+1 ladder.
    def trade_low_bid_5500(self, state, result):
        product = "VEV_5500"
        q = quote_from(state.order_depths.get(product))
        if q.bid is None or q.ask is None:
            return
        ...
        # Stack large bid at 0 (free option) and at 1 (cheap option).
        self.buy(state, product, orders, 0, 100)
        self.buy(state, product, orders, 1, 100)
        # Sell back any longs at any positive ask >= 3.
        if position > 0 and q.ask >= 3:
            self.sell(state, product, orders, q.ask, min(position, 100))
