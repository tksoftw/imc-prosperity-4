"""ROUND_3: SMILE/IV focused trader.

Base is the robust FLIPVOL book. The added edge is option-specific:
fit the IV smile from the current voucher ladder, value each strike from
a leave-one-out surface, then fade one-tick residuals around that surface.
When the whole surface is cheap versus its own slow EMA and spot is
moving, add a small long-gamma bias; when it is rich and quiet, lean
short theta.
"""
...
class Trader:
    ...
    @staticmethod
    def live_tte(timestamp: int) -> float:
        # Public days are consecutive chunks and the real run is the next
        # chunk. Under that clock the final run has roughly two to one
        # model-days left, not the constant T=1 used by older traders.
        return max(0.25, 2.0 - float(timestamp) / 1_000_000.0)
    ...
    def gamma_bias(self, store: Dict, S: float, smile_lv: float) -> float:
        ...
        # Long gamma only when the surface is cheap and spot is actually
        # moving. Short theta only when surface is rich and spot is quiet.
    ...
    def trade_smile_iv_overlay(
        ...
        for strike in (5200,):
            ...
            if q.mid is None or q.spread is None:
                continue
            tte = self.live_tte(state.timestamp)
            fair_iv = self.smile_level_excluding(state, store, S, strike) + smile_offset((strike - S) / 100.0)
            fair_iv = max(IV_LO, min(IV_HI, fair_iv))
            delta = bs_delta(S, strike, tte, fair_iv)
            surface_fair = bs_call(S, strike, tte, fair_iv)
            residual = q.mid - surface_fair
            resid_ema = update_ema(store["emas"], f"{product}_loo_resid", residual, 50)

            # Use surface residual as fair-value mean reversion, then add a
            # small vol-regime convexity tilt. Keep it smaller than spread.
            fair = surface_fair + resid_ema
            fair += max(-1.0, min(1.0, gamma_bias * weights[strike]))
            fair += 0.35 * delta * spot_edge
            fair -= 0.012 * self.position(state, product)

            edge = 1.4 if q.spread <= 2 else min(3.0, 0.8 * q.spread)
            size = 3
            orders = result.setdefault(product, [])
    ...
    def run(self, state: TradingState):
        ...
        self.trade_smile_iv_overlay(state, store, result, S, spot_edge, gamma_bias)
