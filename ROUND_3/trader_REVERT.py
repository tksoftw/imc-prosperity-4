"""ROUND_3: trader_REVERT — stronger one-step mean reversion.
...
single real submission log. Underlying fairs fade the last mid jump.
"""
...
class Trader:
    ...
    def trade_hydro(
        ...
        fair += self.mid_reversion(store, HYDRO, wall, 0.35)
        fair -= 0.05 * position
    ...
    def trade_velvet(
        ...
        fair += self.mid_reversion(store, VELVET, wall, 0.45)
        fair -= 0.005 * position
        if v_active:
            fair += 1.5
        fair += implied_tilt
    ...
    def velvet_spot_edge(
        ...
        fair = 0.7 * fast + 0.3 * 5255.0
        fair += self.mid_reversion(store, VELVET, S, 0.45)
    ...
    def mid_reversion(self, store: Dict, key: str, mid: float, strength: float) -> float:
        prev = store.get("prev_under_mids", {}).get(key)
        if prev is None:
            return 0.0
        move = mid - float(prev)
        return max(-3.0, min(3.0, -strength * move))
    ...
    def run(self, state: TradingState):
        ...
        store["prev_S"] = S
        prev_under = store.setdefault("prev_under_mids", {})
        hq = quote_from(state.order_depths.get(HYDRO))
        if hq.wall_mid is not None:
            prev_under[HYDRO] = hq.wall_mid
        prev_under[VELVET] = S
