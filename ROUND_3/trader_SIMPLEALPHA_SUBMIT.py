...
class Trader(BaseTrader):
    ...
    def trade_hydro(self, state, store, result):
        ...
        if edge > 14.0:
            ...
            target = current
        self._trade_to_target(state, result, HYDRO, target)
    ...
    def trade_velvet(self, state, store, result, v_active, implied_tilt, v4_dir):
        ...
        if edge is None:
            return
        target = self._swing_target(VELVET, edge, self.position(state, VELVET))
        self._trade_to_target(state, result, VELVET, target)
    ...
    def trade_itm_flash(
        ...
        BaseTrader.trade_itm_flash(self, state, result, product, strike, S, surface_shift, wing_signal)
