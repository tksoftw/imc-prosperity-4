"""ROUND_3: trader_MADSCIENTIST — alpha-stack on top of trader_final_OPTIMUM.

Final 3-day backtest: $308,812 vs $206K (trader_final) = +50%.
See notebooks/round3/MADSCIENTIST.txt for the full investigation log.

What actually moved PnL (in order of magnitude):

  1) ULTRA-SOFT POSITION SKEW (the big one, +$60K).
     The original OPTIMUM/trader_final used skew = 0.05-0.06 on every
     option fair value. Combined with bigger 290-cap position limits,
     this aggressive skew was *cutting our quotes below where the book
     was willing to pay*. Dropping every skew to 0.005-0.03 unlocked:
       HYDRO   $97K → $139K   (skew 0.05 → 0.005)
       5300    $24K → $39K    (skew 0.06 → 0.03)
       5400    $7K  → $12K    (skew 0.05 → 0.025)
       VELVET  $73K → $79K    (skew 0.015 → 0.005)
       5000    $2K  → $4K     (skew 0.05 → 0.025)
     Aggressive skew was a *risk* knob, not a *PnL* knob — when the
     book is uninformed (which it is for HYDRO and most OTM options
     per trader_identification.ipynb), skew just leaks alpha.

  2) POSITION LIMIT BUMPS (+$13K). Game caps options at 300; raising
     working caps to 290 (5200/5300, 4000/4500) and 200 (5400/5100/
     5000) lets MM accumulate full inventory before stop-out.

  3) WING-BASKET ATM TILT (~+$0K, kept for OOS robustness).
     Validated alpha: when Wing Seller dumps ≥1 OTM strike, VEV_5200
     mid drifts +0.87 ticks within 100 ticks (t-stat ~22, 280 events).
     We tilt 5200/5300/5400 fairs upward for 100 ticks after each
     basket fire. Drift is real but mostly absorbed by the regular MM,
     so the explicit tilt only books a few hundred S$.

What we tested and dropped:

  * Vertical-spread arb on 5100/5200/5300 — the round-trip bid/ask
    cost (~5-7 ticks) exceeds the 1.5σ entry threshold, net negative.
  * Direct take-on-basket-event lift — paying the full ask spread
    (3 ticks on 5200) burns the 0.87-tick alpha; MM captures it.
  * VEV_4000-trader copy as a VELVET tilt — signal didn't materialize
    in the rust simulator (suspect market_trades reporting differs).
  * Skew = 0.0 — slightly worse than 0.005 (0.005 still helps mean-
    revert long-term inventory drift without choking nearby quotes).
"""
