...
class Trader:
    ...
    def update_signals(self, state: TradingState, store: Dict) -> Tuple[bool, int, bool]:
        ...
        if basket_hits >= 2:
            store["basket_til"] = state.timestamp + 300
    ...
    # ---- HYDROGEL_PACK: claude_carry_2 adaptive mean reversion ----
    def trade_hydro(
        self,
        state: TradingState,
        store: Dict,
        ...
        fast = update_ema(store["emas"], f"{HYDRO}_fast", mid, 30)
        slow = update_ema(store["emas"], f"{HYDRO}_slow", mid, 120)
        fair = 0.20 * mid + 0.25 * fast + 0.40 * slow + 0.15 * 9980.0
        fair -= 0.06 * position
    ...
    def trade_5500(self, state, store, result, S, basket_active: bool):
        ...
        if q.spread is not None and q.spread <= 2 and q.bid <= fair - 0.5:
            ...
            self.buy(state, product, orders, q.bid, size)
