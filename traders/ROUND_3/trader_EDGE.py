"""ROUND_3: trader_EDGE — post-final edge hunt.
...
calibration fuel. Extra layer: portfolio delta guard and wing bag limits.
"""
...
class Trader:
    ...
    def trade_velvet(
        ...
        S_here = q.mid if q.mid is not None else wall
        option_delta = self.option_delta_exposure(state, S_here)
        ...
        if state.timestamp >= 900000:
            hedge_strength = 0.007
        fair -= hedge_strength * option_delta
        if v_active:
            fair += 1.5
        fair += implied_tilt
        fair += 1.0 * v4_dir
    ...
    def option_delta_exposure(self, state: TradingState, S: float) -> float:
        """Approximate call delta inventory so VELVET does not double long."""
        exposure = 0.0
        for strike in (4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500):
            pos = self.position(state, f"VEV_{strike}")
            if pos == 0:
                continue
            if strike <= 4500:
                delta = 1.0
            else:
                delta = bs_delta(S, strike, TTE, 0.033)
            exposure += pos * delta
        return max(-900.0, min(900.0, exposure))
    ...
    def trade_5500(self, state, store, result, S, basket_active: bool):
        ...
        if q.spread is not None and q.spread <= 2 and q.bid <= fair - 0.5:
            ...
            if q.spread >= 2 and q.bid + 1 < q.ask:
                target_bid = q.bid + 1
            if position < 80:
                self.buy(state, product, orders, target_bid, min(size, 80 - position))
