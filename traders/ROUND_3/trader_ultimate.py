"""ROUND_3: trader_ultimate — real-log champion mix.

Built from the matching `imc_logs/ROUND_3/*NAME*.log` files: keep the
robust design from trader_final_3, use OPTIMUM hydro, MADSCIENTIST
VELVET/5300/5400 softness, and EXPERIMENTS residual fades for ATM.
"""
...
class Trader:
    ...
    def trade_hydro(
        ...
        # Real-site ROUND_3 log sits in a lower HYDRO regime. OPTIMUM's
        # fixed 9980 wall-mid anchor beat every other live log.
        fast = update_ema(store["emas"], f"{HYDRO}_fast", wall, 30)
        slow = update_ema(store["emas"], f"{HYDRO}_slow", wall, 120)
        fair = 0.20 * wall + 0.25 * fast + 0.40 * slow + 0.15 * 9980.0
        fair -= 0.05 * position
