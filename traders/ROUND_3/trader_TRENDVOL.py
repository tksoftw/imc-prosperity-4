"""ROUND_3: trader_TRENDVOL — avoid catching falling wing vol.
...
calibration fuel. Wing fair is cut when online extrinsic trend is down.
"""
...
class Trader:
    ...
    def extrinsic_downshift(self, store: Dict, product: str, observed_ext: float) -> float:
        fast = update_ema(store["emas"], f"{product}_ext_fast", observed_ext, 40)
        slow = update_ema(store["emas"], f"{product}_ext_slow", observed_ext, 240)
        # Downtrend in extrinsic means hidden vol regime may be compressing.
        return max(0.0, min(8.0, slow - fast))
    ...
    def trade_5300(self, state, store, result, S, vev_dir):
        ...
        intrinsic = max(S - strike, 0.0)
        observed_ext = q.mid - intrinsic
        downshift = self.extrinsic_downshift(store, product, observed_ext)
        fair = intrinsic + 47.0 - 0.75 * downshift
        fair -= 0.015 * position
        if vev_dir != 0:
            fair += 2.0 * vev_dir

        if q.ask < fair - 2.0 and downshift < 4.0:
    ...
    def trade_5400(self, state, store, result, S):
        ...
        observed_ext = q.mid - intrinsic
        downshift = self.extrinsic_downshift(store, product, observed_ext)
        fair = intrinsic + 16.0 - 0.75 * downshift
    ...
    def trade_5500(self, state, store, result, S, basket_active: bool):
        ...
        fair = intrinsic + ext_blend - 0.50 * downshift
        fair += self.option_micro_reversion(store, product, q, S, 0.08, 0.50)

        if basket_active and q.ask <= fair - 0.5 and downshift < 3.0:
            self.buy(state, product, orders, q.ask, min(-q.ask_vol, 8))
        elif q.ask <= fair - 2.5 and downshift < 3.0:
