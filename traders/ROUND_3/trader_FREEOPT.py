...
class Trader:
    ...
    def trade_5500(self, state, store, result, S, basket_active: bool):
        ...
        # Structural cheap-convexity grab: ask=1 is the minimum paid offer
        # on this voucher. It is not calibrated to a submission log.
        if q.ask <= 1:
            self.buy(state, product, orders, q.ask, min(-q.ask_vol, 25))
        if position > 0 and q.bid is not None and q.bid >= 2:
            self.sell(state, product, orders, q.bid, min(position, q.bid_vol, 25))
