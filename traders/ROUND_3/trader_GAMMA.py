"""ROUND_3: GAMMA — SMILE/IV/Black-Scholes + gamma scalping, max-aggression.

Goal: hit the per-product targets that are theoretically achievable on
days 0/1/2 (HYDRO ≥ 50k/d, VEV_4000/4500 ≥ 25-35k/d, VEV_5000-5200
≥ 30-50k/d) by:

  * HYDROGEL_PACK   — historical-fair MM with a *very* soft position
                       skew (0.005 instead of 0.05) so we hold inventory
                       through mean-reversion cycles. Trader_MS shows
                       this single change is worth ~$30k/day.
  * VELVETFRUIT_EXTRACT — saturated MM (full 200 each side) plus a
                       net-portfolio-delta hedge tilt: every option
                       fill changes our delta exposure, and the VELVET
                       quote ladder shifts to absorb it. That is the
                       gamma-scalp engine.
  * VEV_4000 / 4500 — deep-ITM (delta ≈ 1) with almost no bot flow.
                       Replace the cautious flash-arb with a saturated
                       intrinsic-anchored MM at the full 300 limit,
                       quote edge 1, plus surface-shift flash takes.
  * VEV_5000-5200   — ATM, near-zero bot flow. Aggressive Black-Scholes
                       smile MM at the full 300 limit, take edge 1.0,
                       quote edge 0.5, full quote size every tick.
                       This is where the bulk of unrealised PnL lives:
                       holding gamma into VELVET swings.
  * VEV_5300-5500   — kept exactly as in trader_SMILE_IV (already at or
                       above the target on those strikes).
  * VEV_6000 / 6500 — unchanged (zero-tail absorber).

Every option position carries an explicit Black-Scholes delta. Net
portfolio delta is shoved back toward neutral through VELVET — when
spot rallies and the long-gamma position grows positive delta, we sell
more VELVET; when spot drops we buy. The convexity captured by that
rebalancing IS gamma scalping.

Everything else (smile fitting, leave-one-out IV, micro-reversion,
v_active / vev_dir / basket / v4_dir signals, low-IV trail) is
inherited from trader_SMILE_IV — the existing IV engine is the leader.
"""
...
class Trader:
    ...
    def flatten_options(self, state: TradingState, result: Dict[str, List[Order]]) -> None:
        for product in OPTION_STRIKES:
            sym = f"VEV_{product}"
            pos = self.position(state, sym)
            if pos == 0:
                continue
            q = quote_from(state.order_depths.get(sym))
            orders = result.setdefault(sym, [])
            if pos > 0 and q.bid is not None:
                self.sell(state, sym, orders, q.bid, min(pos, max(1, q.bid_vol), 40))
            elif pos < 0 and q.ask is not None:
                self.buy(state, sym, orders, q.ask, min(-pos, max(1, -q.ask_vol), 40))

    # ---- Signals (same as SMILE_IV) ----
    ...
    # ---- HYDROGEL_PACK: trader_MS soft-skew variant ($170k vs $127k) ----
    def trade_hydro(
        self,
        state: TradingState,
        store: Dict,
        ...
        QUOTE_SIZE = 18
        MIN_SPREAD = 10
        # The single most-impactful change vs SMILE_IV: skew 0.05 → 0.005.
        # Holds inventory through mean-reversion cycles instead of dumping.
        SKEW = 0.005
    ...
    # ---- Net portfolio delta (option positions only, in VELVET units) ----
    def portfolio_option_delta(self, state: TradingState, store: Dict, S: float) -> float:
        total = 0.0
        # smile_level is needed for fair-IV-based deltas; if it isn't ready,
        # use BS deltas with a conservative default IV.
        smile_lv = float(store["emas"].get("smile_level", 0.030))
        for strike in OPTION_STRIKES:
            sym = f"VEV_{strike}"
            pos = self.position(state, sym)
            if pos == 0:
                continue
            iv = max(IV_LO, min(IV_HI, smile_lv + smile_offset((strike - S) / 100.0)))
            d = bs_delta(S, strike, TTE, iv)
            total += pos * d
        return total

    # ---- VELVET: saturated MM + option-Δ hedge tilt (gamma scalp engine) ----
    def trade_velvet(
        self,
        state: TradingState,
        store: Dict,
        ...
        # Hedge tilt: skew the VELVET fair against net option delta so the
        # MM ladder absorbs (instead of doubles) net exposure. With 300×
        # 5200 calls (Δ≈0.5) we sit at +150 Δ units in VELVET-equivalents;
        # this nudges fair down and we sell more VELVET — that IS the
        # gamma scalp leg.
        opt_delta = self.portfolio_option_delta(state, store, S)
        # Coefficient kept small (sub-tick at typical exposures) so the
        # MM is still saturated; it just leans the right way.
        fair -= 0.004 * opt_delta
    ...
    # ---- DEEP-ITM (4000, 4500): proven flash-arb from SMILE_IV ----
    # Tested empirically: saturated MM here loses on d0 because the
    # surface-shift bug only fires occasionally and the rest of the time
    # we're parking large limit orders that get adversely filled. Stick
    # with the conservative flash-arb that scored $28k / $32k.
    def trade_itm_flash(
        self,
        state: TradingState,
        result: Dict[str, List[Order]],
    ...
    # ---- ATM (5000, 5100, 5200): saturated BS-smile MM ----
    # Same observation: ~zero bot-vs-bot flow here. We BE the book.
    def trade_atm_saturated(
        self,
        state: TradingState,
        store: Dict,
        result: Dict[str, List[Order]],
        ...
        # Use leave-one-out IV (other strikes' info) so the surface fair
        # is independent of THIS strike's own mid — crucial for residual
        # detection (otherwise surface_fair tracks q.mid and residual=0).
        loo_iv = self.smile_level_excluding(state, store, S, strike) + smile_offset(moneyness)
        loo_iv = max(IV_LO, min(IV_HI, loo_iv))
        delta = bs_delta(S, strike, TTE, loo_iv)
        loo_fair = bs_call(S, strike, TTE, loo_iv)

        # Intrinsic + observed-ext anchor (smooths the BS noise).
        intrinsic = max(S - strike, 0.0)
        observed_ext = q.mid - intrinsic
        ext_ema = update_ema(store["emas"], f"{product}_ext", observed_ext, 80)
        anchor_ext = loo_fair - intrinsic
        ext_blend = 0.5 * ext_ema + 0.5 * anchor_ext
        fair = intrinsic + ext_blend

        # Spot-edge / gamma overlays
        spot_w = 0.9 if strike == 5000 else (1.0 if strike == 5100 else 0.5)
        fair += spot_w * delta * spot_edge
        fair += max(-1.5, min(1.5, gamma_bias))
        fair += self.option_micro_reversion(store, product, q, S, delta, 0.9)
        fair -= 0.015 * position

        # Aggressive takes — full available top-of-book volume.
        if q.ask <= fair - 1.5:
            self.buy(state, product, orders, q.ask, -q.ask_vol)
        if q.bid >= fair + 1.5:
            self.sell(state, product, orders, q.bid, q.bid_vol)

        # Saturated quoting at fair±1 with full POS_LIMIT each side.
        target_bid = int(math.floor(fair - 1.0))
        target_ask = int(math.ceil(fair + 1.0))
        if 0 < target_bid < target_ask:
            self.buy(state, product, orders, target_bid, POS_LIMITS[product])
            self.sell(state, product, orders, target_ask, POS_LIMITS[product])

    def smile_level_excluding(
        self,
        state: TradingState,
        store: Dict,
        S: float,
        ...
        for strike in (5000, 5100, 5200, 5300, 5400, 5500):
            ...
            iv_smooth = update_ema(store["emas"], f"iv_x_{excluded_strike}_{strike}", iv_obs, 80)
            level = iv_smooth - smile_offset((strike - S) / 100.0)
            w = 1.0 / max(1, q.spread)
            weighted += w * level
            total_w += w
        if total_w == 0:
            return float(store["emas"].get("smile_level", 0.033))
        raw = weighted / total_w
        level = update_ema(store["emas"], f"loo_smile_{excluded_strike}", raw, 80)
    ...
    def gamma_bias(self, store: Dict, S: float, smile_lv: float) -> float:
        prev_s = store.get("prev_S")
        if prev_s is None:
            return 0.0
        move = abs(S - float(prev_s))
        fast_move = update_ema(store["emas"], "spot_abs_move_fast", move, 30)
        slow_move = update_ema(store["emas"], "spot_abs_move_slow", move, 300)
        slow_iv = update_ema(store["emas"], "smile_level_slow", smile_lv, 600)
        iv_gap = smile_lv - slow_iv

        if iv_gap < -0.0010 and fast_move > slow_move + 0.15:
            return min(1.0, (-iv_gap / 0.0020) * min(1.0, fast_move / max(0.5, slow_move)))
        if iv_gap > 0.0012 and fast_move < slow_move - 0.10:
            return -min(0.8, iv_gap / 0.0025)
        return 0.0

    # ---- VEV_5300: extrinsic + filtered vev_dir tilt (unchanged) ----
    ...
    # ---- VEV_5400: extrinsic = 16 OTM MM (unchanged) ----
    ...
    # ---- VEV_5500: basket-seller absorber (unchanged) ----
    ...
    def run(self, state: TradingState):
        ...
        # The smile-level EMA is a precondition for the option BS pricing
        # AND for the option-Δ hedge in trade_velvet, so compute it before
        # touching VELVET.
        smile_lv = self.smile_level(state, store, S)
        gamma_bias = self.gamma_bias(store, S, smile_lv)
        ...
        # VELVET MM uses the fresh smile_level via portfolio_option_delta.
        self.trade_velvet(state, store, result, v_active, implied_tilt, v4_dir, S)
        ...
        # Deep-ITM: proven flash-arb (saturated MM here lost ~$25k vs flash on d0).
        for strike in ITM_STRIKES:
            self.trade_itm_flash(
                state, result, f"VEV_{strike}", strike, S, surface_shift, wing_signal,
            )

        # ATM saturated MM
        for strike in ATM_STRIKES:
            self.trade_atm_saturated(
                state, store, result, f"VEV_{strike}", strike, S,
                smile_lv, spot_edge, gamma_bias,
            )

        # Wing strategies (proven)
