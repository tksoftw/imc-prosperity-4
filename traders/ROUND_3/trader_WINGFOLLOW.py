"""ROUND_3: trader_WINGFOLLOW — follow short-vol wing seller flow.
...
calibration fuel. Wing-seller basket prints bias OTM calls lower.
"""
...
class Trader:
    ...
    def update_signals(self, state: TradingState, store: Dict) -> Tuple[bool, int, bool, int]:
        ...
        for prod in ("VEV_5300", "VEV_5400", "VEV_5500", "VEV_6000", "VEV_6500"):
            ...
            for tr in state.market_trades.get(prod, []):
                px = int(tr.price)
                n = abs(int(tr.quantity))
                if q.bid is not None and px <= q.bid:
                    wing_sell_score += n
                elif q.mid is not None and px < q.mid:
                    wing_sell_score += n
        if wing_sell_score >= 6:
            store["wing_sell_til"] = state.timestamp + 1500
        ...
        wing_sell_active = state.timestamp <= int(store.get("wing_sell_til", -1))
        return v_active, vev_dir, basket_active or wing_sell_active, v4_dir
    ...
    def trade_5300(self, state, result, S, vev_dir, flow_active: bool):
    ...
    def trade_5400(self, state, store, result, S, flow_active: bool):
    ...
    def run(self, state: TradingState):
        ...
        self.trade_5300(state, result, S, vev_dir, basket_active)
        self.trade_5400(state, store, result, S, basket_active)
