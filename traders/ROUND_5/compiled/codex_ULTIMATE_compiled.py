...
#+traders/ROUND_5/codex_ULTIMATE.py
def __build_codex_ULTIMATE():
    from datamodel import OrderDepth, TradingState, Order
    from typing import Dict, List, Tuple
    import json
    import math
    ...
    class Trader:
        ...
        ROBOT_GROUP = ('ROBOT_DISHES', 'ROBOT_IRONING', 'ROBOT_LAUNDRY', 'ROBOT_MOPPING', 'ROBOT_VACUUMING')
        POSITION_LIMIT = 10
        MAX_ORDERS_PER_PRODUCT = 10
        NO_TRADE_ALWAYS = {'GALAXY_SOUNDS_SOLAR_WINDS', 'ROBOT_VACUUMING', 'SNACKPACK_PISTACHIO', 'TRANSLATOR_ASTRO_BLACK'}
        RESCUE_ALWAYS = {'MICROCHIP_TRIANGLE', 'PANEL_1X2', 'TRANSLATOR_SPACE_GRAY'}
        RESCUE_NORMAL = {'OXYGEN_SHAKE_MORNING_BREATH'}
        RESCUE_LATE_REGIME = {'GALAXY_SOUNDS_PLANETARY_RINGS', 'MICROCHIP_OVAL', 'MICROCHIP_SQUARE', 'PANEL_4X4', 'ROBOT_MOPPING', 'TRANSLATOR_GRAPHITE_MIST', 'UV_VISOR_ORANGE', 'UV_VISOR_YELLOW'}
        NO_TRADE_LATE_REGIME = {'GALAXY_SOUNDS_DARK_MATTER', 'GALAXY_SOUNDS_SOLAR_FLAMES', 'PANEL_1X4', 'SLEEP_POD_LAMB_WOOL', 'SLEEP_POD_POLYESTER', 'SLEEP_POD_SUEDE', 'OXYGEN_SHAKE_MORNING_BREATH'}
        ...
        def run(self, state: TradingState):
            ...
            for product in state.order_depths:
                ...
                if self.should_rescue_product(product, late_regime):
                    rescue_orders, product_memory = self.trade_rescue_product(product, state, mids, rescue_memory.get(product, {}))
        ...
        def should_rescue_product(self, product: str, late_regime: bool) -> bool:
            return product in self.RESCUE_ALWAYS or (not late_regime and product in self.RESCUE_NORMAL) or (late_regime and product in self.RESCUE_LATE_REGIME)

        def should_skip_product(self, product: str, late_regime: bool) -> bool:
            return product in self.NO_TRADE_ALWAYS or (late_regime and product in self.NO_TRADE_LATE_REGIME)
        ...
        def trade_rescue_product(self, product: str, state: TradingState, mids: Dict[str, float], memory: Dict) -> Tuple[List[Order], Dict[str, float]]:
    ...
    for __k,__v in list(locals().items()):
        if not __k.startswith('__'):setattr(__m,__k,__v)
    return __m
codex_ULTIMATE=__build_codex_ULTIMATE()
setattr(traders.ROUND_5,'codex_ULTIMATE',codex_ULTIMATE)

Trader = codex_ULTIMATE.Trader
